...
class EnhancedNocturnalAgent:
    ...
    def _extract_papers_from_context(self, api_results: Dict[str, Any], question: str) -> List[Dict[str, Any]]:
        ...
        # If no papers found, check conversation history for recent paper mentions
        if not papers and self.conversation_history:
            # Look for papers in recent assistant responses
            for msg in reversed(self.conversation_history[-5:]):
                if msg.get("role") == "assistant":
                    content = msg.get("content", "")
                    # Try to extract paper info from response
                    # This is a simple heuristic - in real case, we'd need better parsing
                    if "title" in content.lower() and "author" in content.lower():
                        # Paper was mentioned in response
                        pass
